...
args = getResolvedOptions(sys.argv, ["JOB_NAME", "lz_db_name", "lz_table_name", "rz_db_name", "rz_table_name", "fz_db_name", "fz_table_name","lz_error_db", "lz_error_table","lz_error_s3_path", "TempDir"])
...
def extract_conversion_columns():
    ...
    try:
        ...
        DatabaseName=args["fz_db_name"],
        TableName=args["fz_table_name"]
            )
...
def lz2rz():
    ...
    if dyf.count() >0:
        try:
            ...
            refZoneDf = cast_column_datatype(rawZoneDf, timestamp_cols, "timestamp")
            refZoneDf = cast_column_datatype(refZoneDf, date_cols, "date")
            refZoneDf = cast_column_datatype(refZoneDf, int_cols, "int")
            refZoneDf = cast_column_datatype(refZoneDf, bigint_cols, "bigint")
            refZoneDf = refZoneDf.drop(*date_cols)
            refZoneDf = refZoneDf.drop(*int_cols)
            refZoneDf = refZoneDf.drop(*timestamp_cols)
            refZoneDf = refZoneDf.drop(*bigint_cols)
            refZoneDf = update_column_names(refZoneDf)
            
            final_ref_dyf = DynamicFrame.fromDF(refZoneDf, glueContext, "final_ref_dyf")
        ...
        try:
            glueContext.write_dynamic_frame.from_options(
                            frame = final_raw_dyf,
                            connection_type = "s3",    
                            connection_options = {"path": glue_catalog_path(args["fz_db_name"],args["fz_table_name"]), "partitionKeys": ["load_dt"]},
                            format = "parquet")
            print("SUCCESS: data loaded into raw to ref zone")
            '''
            final = glueContext.write_dynamic_frame.from_catalog(
                frame=final_ref_dyf,
                database=args["fz_db_name"],
                table_name=args["fz_table_name"],
                transformation_ctx="write to ref zone")
                '''
        except Exception as e:
            print("Failed to write rz to the Ref Zone.")
            print("Exception: ", traceback.print_exc())
            raise
