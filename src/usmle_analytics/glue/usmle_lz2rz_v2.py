...
args = getResolvedOptions(sys.argv, ["JOB_NAME", "lz_db_name", "lz_table_name", "rz_db_name", "rz_table_name", "lz_error_db", "lz_error_table","lz_error_s3_path", "TempDir", "ssm_param_name"])
...
def extract_conversion_columns():
    ...
    try:
        ...
        bigint_cols = []
        string_cols = []
        audit_columns = ['load_dt', 'boomi_load_dt', 'object_name', 'etl_date','total_records']
        ...
        for meta in glue_catalog_meta:
            ...
            elif meta.get("Type").lower() == "string":
                string_cols.append(meta.get("Name"))
        return   glue_catalog_meta, date_cols, int_cols, timestamp_cols, bigint_cols ,string_cols 
    except Exception as e:
...
def schema_matcher(df):
    ...
    print("dyfCollist",dyfColList)
    glue_catalog_meta = response.get("TableVersions")[0].get("Table").get("StorageDescriptor").get("Columns")
    print("glue_catalog_meta", glue_catalog_meta)
    for meta in glue_catalog_meta:
...
def glue_catalog_path(db, table):
    try:
        client = boto3.client('glue')
        response = client.get_table_versions(DatabaseName=db, TableName=table)
        location = response.get("TableVersions")[0].get("Table").get("StorageDescriptor").get("Location")
        return str(location)
    except Exception as e:
        print("Failed to extract glue s3 path")
        print("Exception: ", e)
        return False
        raise  
    
def get_latest_partition_value(db, tablename):
    try:
        ...
        for partition in response.get('Partitions', []):
            partition_values = partition.get('Values', [])
            if partition_values:
                partition_value = partition_values[0]  # Assuming the partition key is the first value
                if latest_partition_value is None or partition_value > latest_partition_value:
                    latest_partition_value = partition_value
        return str(latest_partition_value)
    except Exception as e:
        print("Failed in get_latest_partition_value")
        print("Exception: ", traceback.print_exc())
        return ""
        raise   


def updated_ssm_param(param_name, value):
    try:
        ...
        print("Failed in updating ssm param store")
        print("Exception: ", traceback.print_exc())
        return False
...
def lz2rz():
    ...
    if dyf.count() >0:
        try:
            ...
            df = df.withColumn("load_dt", lit(ETL_DATE).cast(DateType()))
            df = df.withColumn("filename",input_file_name())
            df = df.withColumn("extracted_file_name", file_name_extractor(col("filename")))
            #extracted boomi summary detail from source file name as fixed length
            df =df.selectExpr("substr(extracted_file_name, LENGTH(extracted_file_name) - INSTR(REVERSE(extracted_file_name), '_') + 2) as total_records", \
                "substr(extracted_file_name,1, LENGTH(extracted_file_name) - INSTR(REVERSE(extracted_file_name), '_')) as obj_and_etl_dt", \
                "*")
            df = df.selectExpr("substr(obj_and_etl_dt,1, LENGTH(obj_and_etl_dt) - INSTR(REVERSE(obj_and_etl_dt), '_') ) as object_name", \
                    "substr(obj_and_etl_dt,LENGTH(obj_and_etl_dt) - INSTR(REVERSE(obj_and_etl_dt), '_') +2 ) as etl_date_summary", \
                    "*")
            df = df.withColumn("etl_date_summary_upd", to_timestamp(col("etl_date_summary"), \
                                                                                    'yyyy-dd-MM HH:mm:ss.SSS'))
            #DF for rawzone datacatalog
            rawZoneDf= df.drop("filename", "etl_date_summary", "extracted_file_name", "object_name", "total_records","obj_and_etl_dt")
            rawZoneDf = rawZoneDf.withColumnRenamed("etl_date_summary_upd", "boomi_load_dt")
            # rawZoneDf = rawZoneDf.partitionBy("load_dt")
            rz_schema ,date_cols, int_cols, timestamp_cols, bigint_cols, string_cols = extract_conversion_columns()
            print("rz_schema", rz_schema)
            print("string columns", string_cols)
            rawZoneDf = cast_column_datatype(rawZoneDf, string_cols, "string")
            rawZoneDf = rawZoneDf.drop(*string_cols)
            rawZoneDf = update_column_names(rawZoneDf)
            final_raw_dyf = DynamicFrame.fromDF(rawZoneDf, glueContext, "final")
            final_raw_dyf.printSchema()
            
            #DF for redshfift boomi_summary audit table
            boomiSummaryDf = df.selectExpr("object_name", "etl_date_summary_upd as etl_date", "cast(total_records as int) as total_records").distinct()
        ...
        try:
            glueContext.write_dynamic_frame.from_options(
                            frame = final_raw_dyf,
                            connection_type = "s3",    
                            connection_options = {"path": glue_catalog_path(args["rz_db_name"],args["rz_table_name"]), "partitionKeys": ["load_dt"]},
                            format = "parquet")
            print("SUCCESS: data loaded into rawzone")
            repair_query =  "msck repair table {0}.{1}".format(args["rz_db_name"], args["rz_table_name"])   
            spark.sql(repair_query)
            # latest_partition = get_latest_partition_value(args["rz_db_name"], args["rz_table_name"])
            is_updated_ssm = updated_ssm_param(args["ssm_param_name"], str(ETL_DATE))
            print("updated SSM param", is_updated_ssm)
        except Exception as e:
